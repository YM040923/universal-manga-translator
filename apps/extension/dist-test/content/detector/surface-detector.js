...
function scoreImage(img, rect) {
    ...
    if (rect.width >= 300 && rect.height >= 300)
        score += 4;
    if (rect.height / Math.max(rect.width, 1) >= 1.1)
        score += 3;
    if (/manga|comic|chapter|page|webtoon|reader/i.test(img.currentSrc || img.src))
        score += 2;
    if (rect.width >= 600)
        score += 1;
    return score;
}
export function detectImageSurfaces(root = document) {
...
//# sourceMappingURL=surface-detector.js.map