...
test("DirectClient reuses a completed content fingerprint when a signed image URL changes", async () => {
  ...
  const fetchImpl = (async (url, init) => {
    ...
    return new Response(JSON.stringify({ choices: [{ message: { content: JSON.stringify({ items: [{ id: item.id, translatedText: "你好" }] }) } }] }), { status: 200, headers: { "content-type": "application/json" } });
  }) as typeof fetch;
  const shared = {
