...
export function maskStyleForRegion(kind: string, width = 0, height = 0, appearance: OverlayAppearance = DEFAULT_SETTINGS.overlayAppearance, sourceText?: string, latinDominant = true): { background: string; borderRadius: string; clipPath: string; textShadow: string } {
  ...
  // whose SOURCE text is hangul/punctuation/short non-Latin on a Latin-dominant
  // page (English-localized manhwa) must stay transparent instead of painting
  // a big white bubble.
  if (kind === "dialogue" && sourceText && latinDominant && looksLikeSfxSourceText(sourceText)) {
    return {
      background: "transparent",
      borderRadius: "0",
      clipPath: "none",
