...
<div class="shell">
  ...
  <main class="grid">
    ...
    <section class="card"><h2>${T.ocrSettings}</h2><label class="row"><span>OCR 服务</span><select id="ocrPreset"><option value="generic">${T.genericOcr}</option></select></label><label class="row"><span>OCR URL</span><input id="ocrUrl" placeholder="https://uapis.cn/api/v1/image/ocr"></label><label class="row"><span>${T.inputMode}</span><select id="ocrInput"><option value="image_base64">image_base64</option><option value="file">file</option></select></label><label class="row"><span>${T.keyPool}</span><textarea id="ocrKeys" placeholder="${T.keyPlaceholder}"></textarea></label><details class="advanced"><summary>高级 OCR 适配参数</summary><label class="row"><span>图片字段</span><input id="ocrImageField" placeholder="image_base64"></label><label class="row"><span>固定字段 JSON</span><textarea id="ocrStaticFields" placeholder='{"need_location":true,"enable_cls":false}'></textarea></label><label class="row"><span>结果路径</span><input id="ocrRegionsPaths" placeholder="words_result,data.words_result"></label><label class="row"><span>文本路径</span><input id="ocrTextPaths" placeholder="words,text,content"></label><label class="row"><span>坐标路径</span><input id="ocrBoxPaths" placeholder="location,box,bbox"></label><label class="row"><span>置信度路径</span><input id="ocrConfidencePaths" placeholder="score,confidence"></label></details><div class="actions"><button id="saveOcr" data-backend-required>${T.saveOcr}</button></div></section>
    <section class="card"><h2>${T.modelSettings}</h2><label class="row"><span>Base URL</span><input id="baseUrl" placeholder="https://api.example.com/v1"></label><label class="row"><span>API Key</span><input id="openAiKey" type="password" placeholder="保存后不回显明文；留空保存=保留后端现有 Key"></label><label class="row"><span>${T.model}</span><select id="model"><option value="gpt-5.4-mini">gpt-5.4-mini</option><option value="gpt-5.5">gpt-5.5</option></select></label><label class="row"><span>${T.targetLanguage}</span><select id="targetLanguage"><option value="zh-CN">${T.zhCn}</option><option value="zh-TW">${T.zhTw}</option></select></label><div class="actions"><button id="saveModel" data-backend-required>${T.saveModel}</button><button id="loadModels" data-backend-required class="ghost">${T.loadModels}</button></div></section>
