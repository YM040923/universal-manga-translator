...
test("OcrTranslatePipeline combines OCR regions with translated text", async () => {
  ...
  const translator: CoreTextTranslator = { translate: async (items: TextTranslationItem[]) => items.map((item) => ({ id: item.id, translatedText: "ä½ å¥½" })) };
  ...
  assert.equal(result.regions[0]?.translatedText, "ä½ å¥½");
